  ...
  return (
    <section className={`py-20 px-4 bg-background ${className}`}>
      <div className="container mx-auto text-center">
        <h2 className="text-3xl md:text-4xl font-bold mb-12 text-foreground">{t("statistics.title")}</h2>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {statistics.map((stat, index) => (
            <div key={index} className="text-center">
              <div className="text-4xl md:text-5xl font-bold text-primary mb-2">{stat.value}</div>
              <p className="text-muted-foreground text-sm md:text-base leading-relaxed">{stat.label}</p>
            </div>
          ))}
        </div>
      </div>
